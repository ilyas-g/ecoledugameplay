<?php
/*
 * wps_thumb - WP Image Resizer v1.3
 *
...
if ( ! function_exists( 'wps_thumb' ) ) {
	...
	function wps_thumb( $url, $params = array(), $single = true ) {
		$class = WPS_Class_Factory::getNewestVersion( 'WPS_Thumb' );
...
/**
 * Class factory, this is to make sure that when multiple wps_thumb scripts
 * are used (e.g. a plugin and a theme both use it), we always use the
 * latest version.
 */
if ( ! class_exists( 'WPS_Class_Factory' ) ) {

	class WPS_Class_Factory {
 ...
 */
add_filter( 'wp_image_editors', 'wps_wp_image_editor' );

// Instead of using the default image editors, use our extended ones
if ( ! function_exists( 'wps_wp_image_editor' ) ) {

	function wps_wp_image_editor( $editorArray ) {
		// Make sure that we use the latest versions
		return array(
			WPS_Class_Factory::getNewestVersion( 'WPS_Image_Editor_GD' ),
			WPS_Class_Factory::getNewestVersion( 'WPS_Image_Editor_Imagick' ),
		);
	}
}
...
if ( ! class_exists( 'WPS_Image_Editor_Imagick_1_3' ) ) {

	WPS_Class_Factory::addClassVersion( 'WPS_Image_Editor_Imagick', 'WPS_Image_Editor_Imagick_1_3', '1.3' );

	class WPS_Image_Editor_Imagick_1_3 extends WP_Image_Editor_Imagick {
...
if ( ! class_exists( 'WPS_Image_Editor_GD_1_3' ) ) {

	WPS_Class_Factory::addClassVersion( 'WPS_Image_Editor_GD', 'WPS_Image_Editor_GD_1_3', '1.3' );

	class WPS_Image_Editor_GD_1_3 extends WP_Image_Editor_GD {
 ...
 * Main Class
 */
if ( ! class_exists( 'WPS_Thumb_1_3' ) ) {

	WPS_Class_Factory::addClassVersion( 'WPS_Thumb', 'WPS_Thumb_1_3', '1.3' );
	...
	class WPS_Thumb_1_3 {
		...
		public static function thumb( $url, $params = array(), $single = true ) {
			...
			if ( defined( 'WPS_THUMB_UPLOAD_DIR' ) ) {
				$upload_dir .= "/" . WPS_THUMB_UPLOAD_DIR;
				$upload_url .= "/" . WPS_THUMB_UPLOAD_DIR;
			} else {
				$upload_dir .= "/wps_thumb";
				$upload_url .= "/wps_thumb";
			}
...
if ( ! is_admin() ) {
	add_filter( 'image_resize_dimensions', 'wps_image_resize_dimensions', 10, 5 );
}

if ( ! function_exists( 'wps_image_resize_dimensions' ) ) {
	function wps_image_resize_dimensions( $payload, $orig_w, $orig_h, $dest_w, $dest_h, $crop = false ) {
...
// so that you won't have to use the function wps_thumb in order to do resizing.
// To make this work, in the WP image functions, when specifying an
// array for the image dimensions, add a 'wps_thumb' => true to
// the array, then add your normal $params arguments.
//
// e.g. the_post_thumbnail( array( 1024, 400, 'wps_thumb' => true, 'grayscale' => true ) );
...
if ( ! function_exists( 'wps_image_downsize' ) ) {
	function wps_image_downsize( $out, $id, $size ) {
		...
		if ( ! array_key_exists( 'wps_thumb', $size ) ) {
			return false;
		}
		if ( empty( $size['wps_thumb'] ) ) {
			return false;
		}
