...
class WPSFramework_Settings extends WPSFramework_Abstract {
    ...
    private function _set_settings_options($settings = array(), $options = array()) {
		if (! empty ( $settings )) {
            ...
            $buttons = array(
                'save' => __("Save", 'our-team-members'),
                'restore' => __("Restore" , 'our-team-members'),
                'reset' => __("Reset All Options", 'our-team-members'),
            );
    ...
    public function validate_save($request){
        ...
        if (isset ( $request ['import'] ) && ! empty ( $request ['import'] )) {
			...
			$add_errors [] = $this->add_settings_error ( esc_html__ ( 'Success. Imported backup options.', 'our-team-members' ), 'updated' );
		}
        
        if (isset ( $request ['resetall'] )) {
			$add_errors [] = $this->add_settings_error(esc_html__('Default options restored.', 'our-team-members'),'updated');
			return;
		}
        ...
        if(isset($request['reset']) && !empty($section_id)){
            ...
            $add_errors[] = $this->add_settings_error(esc_html__('Default options restored for only this section.', 'our-team-members'),'updated');
        }
    ...
    public function get_settings_fields() {
        ...
        echo '<input type="hidden" class="wpsf-reset" name="wpsf-section-id" value="'. esc_attr($this->current_section()) .'" />';
        echo '<input class="wpsf_parent_section_id" type="hidden" name="wpsf-parent-section-id" value="'.esc_attr($this->current_section('parent')).'" /> ';
    ...
    public function get_settings_buttons() {
        ...
        if($this->settings['buttons']['save'] !== false){
            ...
            submit_button ( esc_html($text), 'primary wpsf-save', 'save', false, array ('data-save' => esc_html__ ( 'Saving...', 'our-team-members' )));    
        }
