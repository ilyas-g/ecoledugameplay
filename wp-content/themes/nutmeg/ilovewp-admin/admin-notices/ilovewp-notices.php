...
class nutmeg_notice {
	...
	private function is_dismiss_notice() {
		return apply_filters( 'nutmeg_' . $this->name . '_notice_dismiss', true );
	}
	...
	public function get_notices() {
		...
		$ilovewp_theme_admin_notices = get_option( 'nutmeg_admin_notices' );
	...
	public function hide_notices() {
		...
		if ( isset( $_GET['nutmeg-hide-notice'] ) && isset( $_GET['_nutmeg_notice_nonce'] ) ) {
			
			if ( ! wp_verify_nonce( wp_unslash( $_GET['_nutmeg_notice_nonce'] ), 'nutmeg_hide_notices_nonce' ) ) {
				wp_die( __( 'Action failed. Please refresh the page and retry.', 'nutmeg' ) );
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( __( 'Cheatin&#8217; huh?', 'nutmeg' ) );
			}

			if ( $_GET['nutmeg-hide-notice'] ) {
				
				$hide_notice_id = sanitize_text_field( wp_unslash( $_GET['nutmeg-hide-notice'] ) );
				...
				if ( isset($run_update) ) {
					update_option( 'nutmeg_admin_notices', $theme_admin_notices );
				}
